...
function solucionar(){
    ...
    let paso1 = (`f1(${X}) = (((${X} - ${X1}) / (${X0} - ${X1})) * (${fX0})) + (((${X} - ${X0}) / (${X1} - ${X0})) * (${fX1}))`);
    let paso2 = (`f1(${X}) = (((${X - X1}) / (${X0 - X1})) * (${fX0})) + (((${X - X0}) / (${X1 - X0})) * (${fX1}))`);
    let paso3 = (`f1(${X}) = ((${((X - X1) * (fX0))}) / (${X0 - X1})) + ((${((X - X0) * (fX1))}) / (${X1 - X0})))`);
    let paso4 = (`f1(${X}) = ((${((X - X1) * (fX0)) / (X0 - X1)}) + (${((X - X0) * (fX1)) / (X1 - X0)}))`);
    let paso5 = (`f1(${X}) = ` + ((((X - X1) * (fX0)) / (X0 - X1)) + (((X - X0) * (fX1)) / (X1 - X0))));
